...
Connection.prototype.getEventInfo = function(sport, startDateTime, endDateTime, response, callback) {
    var query = this.connection.query(
        "SELECT ev.event, ev.gender "+
        "FROM "+this.eventsTable+" AS ev "+
        "WHERE ev.sport = ? AND "+
        "ev.startDateTime = ? AND "+
        "ev.endDateTime = ?", [sport, startDateTime, endDateTime]);
...
Connection.prototype.getEventVideo = function(sport, startDateTime, endDateTime, response, callback) {
    var query = this.connection.query(
        "SELECT ev.video "+
        "FROM "+this.eventsTable+" AS ev "+
        "WHERE ev.sport = ? AND "+
...
Connection.prototype.getEvents = function(gender, sport, response, callback) {
    console.log('Get these events');
    console.log(this.sportsTable);
    var query = this.connection.query(
        "SELECT ev.event, ev.startDateTime, ev.endDateTime, sp.hashtag_twitter, sp.hashtag_weibo "+
