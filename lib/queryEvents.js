...
Connection.prototype.getEventInfo = function(id, response, callback) {
    var query = this.connection.query(
        "SELECT ev.event, ev.gender, ev.sport "+
...
Connection.prototype.getEvent = function(id, callback) {
  var self = this;
  ...
  query.on('result', function(result) {
    response.startDateTime = result['startDateTime'];
    response.endDateTime = result['endDateTime'];
    response.topics = result['twitter'];

    if(result['weibo'] !== null) {
      response.topics += ','+result['weibo'];
    }
  });
...
Connection.prototype.queryEventList = function(callback) {
  var self = this;
  var response = {};
  var events = [];

  var query = self.connection.query(
    "SELECT `id`, `startDateTime`, `desc`, `endDateTime`, `event`, `sport`, `gender`, `keywords_twitter` AS twitter, `keywords_weibo` AS weibo "+
    "FROM "+self.eventsTable+" "+
    "WHERE `keywords_weibo` IS NOT NULL OR `keywords_twitter` IS NOT NULL");

  console.log(query.sql);

  query.on('err', function(err) {
    console.log("Error in queryEventList: "+err);
  });
