...
Connection.prototype.getAllEvents = function(response, callback) {
    var query = this.connection.query("SELECT sport, event, startDateTime, endDateTime, gender, eventDate FROM events WHERE sport = 'Gymnastics (Artistic)'");
    ...
    query.on('result', function(result) {
        var eventElement = {
            "sport": result['sport'],
            "event": result['event'],
            "startDateTime": result['startDateTime'].toMysqlFormat(),
            "endDateTime": result['endDateTime'].toMysqlFormat(),
