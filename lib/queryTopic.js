...
Connection.prototype.queryData = function(startDateTime, endDateTime, keyword, step, response, callback) {
    ...
    var query = self.connection.query(
        ...
        "WHERE "+
            ...
            "(tt.`hashtag` = ? OR "+
            "ut.`user` = ?) "+
        "GROUP BY year, month, day, hour, minute", [startDateTime.toMysqlFormat(), endDateTime.toMysqlFormat(), keyword, keyword]);
 ...
 */
Connection.prototype.getFrequency = function(interval, keyword, startDateTime, endDateTime, response, callback) {
    var query = this.connection.query(
        "SELECT EXTRACT(YEAR FROM wo.`dateTime`) AS year, "+
               "EXTRACT(MONTH FROM wo.`dateTime`) AS month, "+
               "EXTRACT(DAY FROM wo.`dateTime`) AS day, "+
               "EXTRACT(HOUR FROM wo.`dateTime`) AS hour, "+
               "EXTRACT(MINUTE FROM wo.`dateTime`) DIV "+interval+" * "+interval+" AS minute,"+
               "COUNT(*) AS frequency "+
        ...
        "WHERE `dateTime` > '"+startDateTime.toMysqlFormat()+"' AND "+
              ...
              "(tt.`hashtag` = ? OR "+
              "ut.`user` = ?) "+
        "GROUP BY year, month, day, hour, minute", [keyword, keyword]);
