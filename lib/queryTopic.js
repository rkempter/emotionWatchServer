...
exports.createConnection = function(config) {
  console.log('create new topic connection');
 ...
 * @param callback
 */
Connection.prototype.queryData = function(startDateTime, endDateTime, network, keyword, step, response, callback) {
    
    var self = this;
    var minutes = step / 60;
    var hour = minutes / 60;
    var networkVal = getNetworkValue(network);
    ...
    var query = self.connection.query(
        ...
        "WHERE "+
            ...
            "wo.`hashtag` IN (?) AND "+
            "wo.`networkType` IN (?) "+
        "GROUP BY year, month, day, hour, minute, second", [startDateTime.toMysqlFormat(), endDateTime.toMysqlFormat(), keyword, networkVal]);
    ...
    query.on('result', function(row) {
        ...
        for(var index in row) {
            if(!isNaN(row[index])) {
              ...
              emotions[index.toLowerCase()] = value;
            }
        }
 ...
 */
Connection.prototype.getFrequency = function(startDateTime, endDateTime, network, keyword, step, response, callback) {
    ...
    var query = this.connection.query(
        ...
        "WHERE `dateTime` > '"+startDateTime.toMysqlFormat()+"' AND "+
              ...
              "wo.`hashtag` IN (?) AND "+
              "wo.`networkType` IN (?) "+
        "GROUP BY year, month, day, hour, minute, second", [keyword, networkVal]);
    ...
    query.on('result', function(result) {
        ...
        response[dateTime] = {};
        response[dateTime].frequency = result.frequency;
        ...
        response[dateTime].emotion = getMaximumLabel(result);
    });
...
Connection.prototype.queryTweets = function(startDateTime, endDateTime, network, keyword, response, callback) {
    var self = this;
    var networkVal = getNetworkValue(network);
    ...
    var query = self.connection.query(
        ...
        "WHERE wo.`dateTime` >= ? AND "+
              ...
              "wo.`contempt` != 0 OR "+
              "wo.`anger` != 0) "+
        "ORDER BY RAND() LIMIT 10", [startDateTime.toMysqlFormat(), endDateTime.toMysqlFormat(), keyword, networkVal]); 
