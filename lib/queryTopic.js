    ...
    if(index > 0) {
        ...
        query.on('end', function() {
            ...
            for(var emotionIndex in data) {
                ...
                var avgValue = 2 * data[emotionIndex] / resultCount;//self.mapValue(sum, emotionIndex, resultCount, median);
                if(avgValue > 1) {
                    avgValue = 1;
                }
 ...
 */
Connection.prototype.getFrequency = function(interval, startTime, endTime, response, callback) {
    var query = this.connection.query("
        SET @intervale = ?;
        SELECT COUNT(*) as frequency
        FROM ?
        WHERE dateTime > ? AND dateTime < ?
        GROUP BY EXTRACT(HOUR FROM dateTime), EXTRACT(MINUTE FROM dateTime) DIV @intervale * @intervale
    ", [interval, this.tweetTable, startDateTime, endDateime]);
...
Connection.prototype.queryTweets = function(startDateTime, endDateTime, keyword, emotion, response, callback) {
    ...
    if(undefined !== emotion) {
        var query = self.connection.query("SELECT wo.`user` AS user, wo.`text` AS text, wo.`datetime` AS dateTime, wo.`id` AS id FROM "+self.tweetTable+" AS wo, "+self.emotionTable+" AS wc WHERE wc.`id` = wo.`id` AND wc.`"+emotion+"` > 0 AND wo.`dateTime` >= ? AND wo.`dateTime` < ? LIMIT 100", [startDateTime.toMysqlFormat(), endDateTime.toMysqlFormat()]);
    } else {
        var query = self.connection.query("SELECT wo.`user` AS user, wo.`text` AS text, wo.`datetime` AS dateTime, wo.`id` AS id  FROM "+self.tweetTable+" AS wo WHERE wo.`dateTime` >= ? AND wo.`dateTime` < ? LIMIT 100", [startDateTime.toMysqlFormat(), endDateTime.toMysqlFormat()]);   
    }
