 ...
 * @param max
 */
Connection.prototype.mapValue = function(value, index, count) {
    var self = this;
    value = value / count;
    console.log("Index: "+index+", value: "+value+", median: "+this.medians[index]+", maxValue:"+self.maxValues[index]);
    if(value > this.medians[index]) {
        console.log(0.5 + 0.5 * value / self.maxValues[index]);
      return 0.5 + 0.5 * value / self.maxValues[index];
    } else {
        console.log(value / self.medians[index]);
      return 0.5*value / self.medians[index];
    }
}
    ...
    if(index > 0) {
        ...
        query.on('end', function() {
            ...
            for(var emotionIndex in data) {
                avgValue = self.mapValue(data[emotionIndex], emotionIndex, resultCount, median);
